...
export default function LeadForm({ type = 'personal', compact = false }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" noValidate={false}>
      <h3 className="text-xl font-black text-slate-950">{isBusiness ? 'Business Consultation' : 'Book Free Review'}</h3>
      ...
      <div className={`mt-5 grid gap-4 ${compact ? '' : 'md:grid-cols-2'}`}>
        ...
        <Field label={isBusiness ? 'Insurance of Interest' : 'Insurance I need help with'} name="interest">
          <Select name="interest" options={isBusiness ? ['Keyman', 'Employer-Employee', 'Group Health', 'Group Term', 'Group Accidental', 'All of the above'] : ['Health', 'NRI Health', 'Senior Citizen Health', 'Super Top-Up', 'Term Life', 'Accident', 'Vehicle', 'Complete Review']} />
        </Field>
      ...
      >
        {isBusiness ? 'Business Consultation' : 'Book Free Review'}
      </button>
    </form>
  )
}
