...
const nav = [
  ...
  ['Free Insurance Health Check-Up', '/#free-checkup'],
...
export default function Navbar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 backdrop-blur">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 lg:px-8" aria-label="Primary navigation">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <CTAButton to="/#lead-forms">Book Review</CTAButton>
            <CTAButton to="/#lead-forms" variant="secondary">Business Consultation</CTAButton>
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-2 border-t border-white/15 bg-slate-950 text-xs font-bold text-white shadow-2xl md:hidden">
        <Link to="/#lead-forms" className="flex items-center justify-center gap-1 py-3"><PhoneCall size={15} /> Book Review</Link>
        <a href={`https://wa.me/${placeholders.whatsapp}`} className="flex items-center justify-center gap-1 border-l border-white/10 py-3"><MessageCircle size={15} /> WhatsApp Advisor</a>
      </div>
    </>
  )
}
